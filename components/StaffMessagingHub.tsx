...
// Added MessageSquare to lucide-react imports to fix missing name error on line 172
import { Send, Bot, Phone, Video, CheckCheck, Loader2, Sparkles, Mic, Users, ArrowRight, Activity, ChevronLeft, Signal, MoreVertical, MessageSquare } from 'lucide-react';
...
export const StaffMessagingHub: React.FC<StaffMessagingHubProps> = ({ chatThreads, setChatThreads }) => {
    ...
    const [viewMode, setViewMode] = useState<'list' | 'chat'>('list'); // Mobile view state
    ...
    useEffect(() => { if (viewMode === 'chat') scrollToBottom(); }, [chatThreads, selectedAgent.id, viewMode]);
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        if (selectedAgent.id === 'team-hub') {
            setActiveTypingAgents(['James', 'Elena']);
            setTimeout(() => setActiveTypingAgents(['Marcus']), 1500);
        }
    ...
    return (
        <div className="fixed inset-x-0 bottom-0 top-16 md:static h-auto md:h-[calc(100vh-120px)] flex bg-slate-950 border-t md:border border-white/5 md:rounded-[2.5rem] overflow-hidden shadow-2xl font-sans z-30">
            
            {/* LEFT PANE: ROSTER (Hidden on mobile if in 'chat' mode) */}
            <div className={`${viewMode === 'chat' ? 'hidden' : 'flex'} lg:flex w-full lg:w-80 border-r border-white/5 flex-col shrink-0 bg-slate-900/50 backdrop-blur-3xl z-10`}>
                <div className="p-6 border-b border-white/5 bg-slate-950/40">
                    <h2 className="text-xl font-black text-white flex items-center gap-3 uppercase tracking-tighter italic">
                        <Users className="w-5 h-5 text-indigo-500" /> War Room
                    </h2>
                    <div className="mt-2 flex items-center gap-2 text-[9px] font-black text-cyan-500 uppercase tracking-widest">
                    ...
                    {INITIAL_STAFF.map(agent => (
                        ...
                        >
                            <div className="relative shrink-0">
                                <div className={`w-10 h-10 rounded-xl border-2 transition-all duration-500 overflow-hidden ${selectedAgent.id === agent.id ? 'border-white/20' : 'border-white/5'}`}>
                            ...
                            <div className="text-left min-w-0 flex-1">
                                <span className={`font-black text-xs uppercase truncate block ${selectedAgent.id === agent.id ? 'text-white' : 'text-slate-200'}`}>{agent.name}</span>
                                <p className={`text-[8px] font-black uppercase tracking-[0.2em] ${selectedAgent.id === agent.id ? 'text-indigo-200' : 'text-slate-500'}`}>{agent.role}</p>
                            </div>
                        </button>
                    ))}
                </div>
            ...
            {/* CENTER PANE: CHAT (Hidden on mobile if in 'list' mode) */}
            <div className={`${viewMode === 'list' ? 'hidden' : 'flex'} lg:flex flex-1 flex flex-col bg-slate-950 relative z-10 transition-all`}>
                {/* Chat Header */}
                <div className="h-16 md:h-20 border-b border-white/5 px-4 md:px-8 flex items-center justify-between bg-slate-900/40 backdrop-blur-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="w-9 h-9 rounded-xl bg-slate-950 border border-white/10 flex items-center justify-center shrink-0">
                        ...
                        <div className="min-w-0">
                            <h3 className="font-black text-white text-sm md:text-lg flex items-center gap-2 uppercase tracking-tighter italic truncate">
                                {selectedAgent.name}
                                <span className="hidden sm:inline-block bg-indigo-500/10 text-indigo-400 text-[8px] px-2.5 py-0.5 rounded-full font-black uppercase tracking-widest border border-indigo-500/20">Synced</span>
                            </h3>
                            <p className="text-[8px] md:text-[9px] text-slate-500 font-bold uppercase tracking-widest truncate">{selectedAgent.description}</p>
                        </div>
                    </div>
                    <div className="flex gap-1">
                        <button className="p-2 bg-slate-800/50 rounded-xl text-slate-400"><Phone className="w-4 h-4" /></button>
                        <button className="p-2 bg-slate-800/50 rounded-xl text-slate-400"><Video className="w-4 h-4" /></button>
                    </div>
                </div>

                {/* Messages Scroller */}
                <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-6 md:space-y-8 custom-scrollbar bg-[radial-gradient(circle_at_top,_rgba(79,70,229,0.03),_transparent)]">
                    {currentMessages.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-700 opacity-30">
                            <MessageSquare className="w-12 h-12 mb-4" />
                            <p className="text-xs font-black uppercase tracking-widest italic">Initialize Comm Sequence</p>
                        </div>
                    ) : (
                        currentMessages.map(msg => (
                            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                                <div className={`max-w-[85%] md:max-w-[75%] flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                                    <div className={`px-5 py-4 rounded-2xl md:rounded-[2rem] text-xs md:text-sm shadow-2xl relative border ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none border-indigo-500' : 'bg-slate-900 text-slate-200 rounded-tl-none border-white/5'}`}>
                        ...
                        <div className="flex flex-col gap-3">
                            {activeTypingAgents.length > 0 ? (
                                activeTypingAgents.map(name => (
                                    <div key={name} className="flex items-center gap-3 text-[9px] font-black uppercase text-cyan-500 italic animate-pulse">
                                        <Loader2 className="w-3.5 h-3.5 animate-spin" /> {name} formulating strategy...
                                    </div>
                                ))
                            ) : (
                ...
                {/* Input Area */}
                <div className="p-4 md:p-8 bg-slate-950 border-t border-white/5 shadow-[0_-10px_30px_rgba(0,0,0,0.3)]">
                    <form onSubmit={handleSend} className="relative flex items-center gap-2 bg-slate-900/50 p-1 md:p-2 rounded-2xl md:rounded-[2rem] border border-white/5 shadow-inner">
                        <button type="button" className="p-3 text-slate-500 hover:text-indigo-400 transition-colors shrink-0"><Mic className="w-5 h-5" /></button>
                        <input 
                            value={input}
                            onChange={e => setInput(e.target.value)}
                            placeholder={isTyping ? "Transmitting..." : `Brief ${selectedAgent.name.toUpperCase()}...`}
            ...
            {/* RIGHT PANE: ANALYTICS (Hidden on screens smaller than 1280px) */}
            <div className="hidden xl:flex w-80 border-l border-white/5 flex-col shrink-0 bg-slate-900/30 backdrop-blur-3xl z-10 p-8 space-y-10">
                <div>
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                        <Activity className="w-4 h-4 text-cyan-400" /> Live Matrix
                    </h3>
                    <div className="space-y-6">
                        {[
                            { label: 'Latency', val: '12ms', color: 'text-green-500' },
                            { label: 'Token Yield', val: '+5.2%', color: 'text-cyan-400' },
                            { label: 'Synced Nodes', val: '152', color: 'text-slate-300' }
                        ].map((stat, i) => (
                            <div key={i}>
                ...
                <div className="space-y-4">
                     <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4">Active Logic</h3>
                     <div className="bg-indigo-600/10 border border-indigo-500/20 p-5 rounded-[2rem] shadow-inner group cursor-pointer hover:bg-indigo-600/20 transition-all">
                         <div className="flex items-center gap-2 mb-3">
                             ...
                             <span className="text-[9px] font-black uppercase text-indigo-400">Optimized</span>
                         </div>
                         <p className="text-[10px] text-slate-300 leading-relaxed font-bold italic">"Expand H2 focus to Sync Licensing for High-Fidelity Electronic tracks."</p>
                         <button className="mt-4 flex items-center gap-2 text-[9px] font-black uppercase text-indigo-400 group-hover:gap-4 transition-all">Apply <ArrowRight className="w-3 h-3"/></button>
                     </div>
                </div>
                ...
                <div className="mt-auto">
                    <div className="p-4 bg-slate-950 border border-white/5 rounded-2xl">
                         ...
                         <p className="text-[8px] font-bold text-slate-600 font-mono">HASH: sf_940_p_rev2</p>
                    </div>
                </div>
            </div>
        </div>
