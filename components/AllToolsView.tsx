...
import { 
    ...
    Vote, Activity, Lock, CheckCircle2, ChevronRight, Sparkles, LayoutGrid
} from 'lucide-react';
...
export const AllToolsView: React.FC<AllToolsViewProps> = ({ stats, onNavigate, onUpgrade }) => {
    ...
    const checkMilestone = (milestone: string) => {
        ...
        switch(milestone) {
            case 'first_asset': return { locked: xp <= 0, reason: 'Requires First Asset' };
            case 'reputation_500': return { locked: xp < 500, reason: 'Requires Level 2 (500 XP)' };
            case 'reputation_1000': return { locked: xp < 1000, reason: 'Requires Level 3 (1000 XP)' };
            case 'reputation_2000': return { locked: xp < 2000 && !isPro, reason: 'Requires Level 4 or Pro' };
            case 'pro_only': return { locked: !isPro, reason: 'Requires Artist Pro Plan' };
        ...
        <div className="space-y-12 pb-24 animate-in fade-in duration-700">
            <div className="max-w-3xl">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-lg">
                    The complete Sound Merge architectural suite. Unlocked features are ready for deployment; locked nodes require higher reputation or institutional upgrades.
                </p>
            </div>
            ...
            {Object.entries(categorizedTools).map(([catKey, tools]) => (
                <div key={catKey} className="space-y-6">
                    <div className="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-4">
                        ...
                        <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-widest italic">{catKey} Suite</h2>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {tools.map(tool => {
                            const { locked, reason } = checkMilestone(tool.milestone);
                            const IconComponent = ICON_MAP[tool.icon] || Music;
                            
                            return (
                                <div 
                                    key={tool.id} 
                                    className={`relative bg-white dark:bg-slate-900 rounded-[2rem] border p-8 transition-all group overflow-hidden ${
                                        locked 
                                    ...
                                    onClick={() => !locked ? onNavigate(tool.id) : tool.milestone === 'pro_only' ? onUpgrade() : null}
                                >
                                    {locked && (
                                    ...
                                    <div className="flex flex-col h-full">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-colors ${
                                            locked ? 'bg-slate-100 dark:bg-slate-800 text-slate-500' : 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 group-hover:bg-cyan-500 group-hover:text-white'
                                        }`}>
                                        ...
                                        <div className="mt-auto flex justify-between items-center">
                                            {locked ? (
                                                <button className="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-1">
                                                    Locked <ChevronRight className="w-3 h-3" />
                                                </button>
                                            ) : (
                                                <button className="bg-slate-950 dark:bg-white text-white dark:text-slate-950 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] group-hover:scale-105 transition-transform">
                                                    Launch Module
                                                </button>
                                            )}
                                            {!locked && tool.ai && (
                                                <span className="text-[8px] font-black uppercase text-purple-500 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">AI Node</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
